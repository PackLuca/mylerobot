...
@PreTrainedConfig.register_subclass("diffusion1")
@dataclass
class DiffusionConfig(PreTrainedConfig):
    ...
    Args:
        ...
        # ========== SDE Framework Parameters ==========
        # SDE type: "vp" (Variance Preserving), "ve" (Variance Exploding), "subvp" (sub-Variance Preserving)
        sde_type: str = "vp"
        # Number of discretization steps for SDE
        sde_num_steps: int = 1000
        # Whether to use continuous time SDE (vs discrete)
        sde_continuous: bool = True
        # Reduce mean for score matching loss
        sde_reduce_mean: bool = True
        # Use likelihood weighting for loss
        sde_likelihood_weighting: bool = True
        # EPS for numerical stability
        sde_eps: float = 1e-5

        # Sampling method: "euler" (Euler-Maruyama), "ode" (Probability Flow ODE), "pc" (Predictor-Corrector)
        sampling_method: str = "euler"
        # Number of sampling steps
        sampling_steps: int = 100

        # Legacy DDPM parameters (kept for compatibility, but SDE is recommended)
    ...
    # Legacy DDPM parameters (for fallback/compatibility)
    noise_scheduler_type: str = "DDPM"
    num_train_timesteps: int = 100
    beta_schedule: str = "squaredcos_cap_v2"
    beta_start: float = 0.0001
